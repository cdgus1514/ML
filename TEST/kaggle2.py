...
sample = pd.read_csv("C:/Users/bitcamp/Downloads/cat-in-the-dat/sample_submission.csv")
...
for train_index, test_index in skf.split(train, train.target.values):
    ...
    es = callbacks.EarlyStopping(monitor='val_auc', min_delta=0.001, patience=8, verbose=1, mode='max', baseline=None, restore_best_weights=True)

    # rlr = callbacks.ReduceLROnPlateau(monitor='val_auc', factor=0.5, patience=3, min_lr=1e-6, mode='max', verbose=1)
    rlr = callbacks.ReduceLROnPlateau(monitor='val_auc', factor=0.2, patience=4, min_lr=1e-6, mode='auto', cooldown=1, verbose=1)
